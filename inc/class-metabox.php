...
class Odin_Metabox {
	...
	public function scripts() {
		...
		if ( in_array( $screen->id, $this->get_post_type() ) ) {
			...
			wp_localize_script(
				...
				'odinAdminParams',
				array(
					'galleryTitle'  => __( 'Add images in gallery', 'wpMachine' ),
					'galleryButton' => __( 'Add in gallery', 'wpMachine' ),
					'galleryRemove' => __( 'Remove image', 'wpMachine' ),
					'uploadTitle'   => __( 'Choose a file', 'wpMachine' ),
					'uploadButton'  => __( 'Add file', 'wpMachine' ),
				)
			);
		}
	}
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button odin-upload-button" type="button" value="%3$s" />', $id, esc_url( $current ), __( 'Select file', 'wpMachine' ), $this->build_field_attributes( $attrs ) );
	}
	/**
	 * Image field.
	 *
	...
	protected function field_image_plupload( $id, $current ) {
		$html = '<div class="odin-gallery-container">';
			...
			$html .= sprintf( '<p class="odin-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Add images in gallery', 'wpMachine' ) );
		$html .= '</div>';
		echo $html;
	}
	/**
